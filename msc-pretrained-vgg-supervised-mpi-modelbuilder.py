...
callback_list = [keras.callbacks.ModelCheckpoint(filepath='Training_Models/model_vgg16_supervised_mpi.h5',monitor='val_loss',save_best_only=True),keras.callbacks.TensorBoard(log_dir='Logs')]
...
history =depth_mapper.fit_generator(multi_train_generator(train_left_datagen,train_right_datagen,train_target_datagen),steps_per_epoch=900,epochs=1000,validation_data=multi_validation_generator(validation_left_datagen,validation_right_datagen,validation_target_datagen),validation_steps=100,callbacks=callback_list)
