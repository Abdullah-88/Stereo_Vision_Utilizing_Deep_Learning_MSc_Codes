...
pre_trained_inceptionv3_left = conv_base(input_img_left)
left_out = pre_trained_inceptionv3_left

# right convnet

input_img_right = Input(shape=(750, 750, 3))
pre_trained_inceptionv3_right = conv_base(input_img_right)
right_out = pre_trained_inceptionv3_right
...
upsample_first = layers.UpSampling2D(size=(2, 2), data_format=None, interpolation='nearest')(decoder_conv_first)
...
upsample_third = layers.UpSampling2D(size=(4, 4), data_format=None, interpolation='nearest')(decoder_conv_third)
...
callback_list = [keras.callbacks.ModelCheckpoint(filepath='Training_Models/model_inception_supervised_kitti.h5',
                                                 monitor='val_loss', save_best_only=True),
                 keras.callbacks.TensorBoard(log_dir='Logs')]

depth_mapper.compile(optimizer=optimizers.RMSprop(lr=1e-6), loss='binary_crossentropy', metrics=['acc'])
...
def multi_train_generator(train_left_datagen, train_right_datagen, train_target_datagen):
    train_left_generator = train_left_datagen.flow_from_directory(train_left_dir, target_size=(750, 750), batch_size=1,
    ...
    train_target_generator = train_target_datagen.flow_from_directory(train_target_dir, target_size=(704, 704),
                                                                      batch_size=1, shuffle=True,class_mode=None)
    while True:
...
def multi_validation_generator(validation_left_datagen, validation_right_datagen, validation_target_datagen):
    ...
    validation_target_generator = validation_target_datagen.flow_from_directory(validation_target_dir,
                                                                                target_size=(704, 704), shuffle=True,batch_size=1,
                                                                                class_mode=None)
    while True:
...
history = depth_mapper.fit_generator(
    multi_train_generator(train_left_datagen, train_right_datagen, train_target_datagen), steps_per_epoch=150,
    epochs=1000, validation_data=multi_validation_generator(validation_left_datagen, validation_right_datagen,
                                                          validation_target_datagen), validation_steps=50,
